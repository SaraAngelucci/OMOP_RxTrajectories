...
├── main.py                              Two-experiment orchestrator
├── make_plots.py                        Silhouette figure for LaTeX
├── notebooks/
...
Python **3.10 or newer** and a JVM (for PySpark) are required. (The pipeline uses
PEP 604 `X | Y` type-union syntax, which is unavailable in 3.9.) `main.py` and the
test suite pin Spark's Python workers to the driver interpreter via
`PYSPARK_PYTHON`, so always run them with the project virtual environment's
`python` (e.g. `./.venv/bin/python`). Two dependency files are provided:
...
`scikit-learn` is included in both files (required for the Calinski-Harabasz and Davies-Bouldin validation indices in `cluster_validation.py`).
...
Runs **Experiment 1–2** with `config/config_synthetic.yaml`. Output prefix is taken from YAML `project.output_dir` (typically `outputs/synthetic/`).

| Step | Behaviour |
|------|-----------|
| 1 Baseline | **Full stratified silhouette** (cosine distance) over `k ∈ {2,…,6}` as a model-selection diagnostic; fixes the operational partition at `clustering.final_k` (default 4); writes `baseline_cohort_*.parquet` and `baseline_cohort_silhouette_grid.csv`. |
...
Expect minutes on \(N≈10^3\); tens of GPU-free hours may apply at \(N≈50{,}000\).

If the baseline run fails with `SparkOutOfMemoryError`, increase `SPARK_DRIVER_MEMORY`.
...
Expected output: ten primary-cohort logic checks (stable mono, polypharmacy, restart, acute exclusion, right-censoring, cluster column typing, acute clustering exclusion, stable feature sanity, poly ordering, Tier A single-era eligibility), four edge-case checks (single-person sentinel cluster $-1$, overlapping eras, boundary-spanning eras, multi-course acute below the Tier B threshold), and the message `VALIDATION COMPLETE. Pipeline is theoretically sound.` Optionally run under pytest: `pytest tests/test_validation_cohort.py`.
...
| `analysis.maintenance_min_total_days` | 56 | Tier B: minimum cumulative era-days (across ≥2 eras) for maintenance eligibility |
| `analysis.maintenance_single_era_min_days` | 180 | Tier A: a single continuous era ≥ this many days is maintenance-eligible (6-month chronic-use convention) |
...
| `clustering.k_grid` | `[2, 3, 4, 5, 6]` | Candidate `k` for the silhouette diagnostic sweep |
| `clustering.final_k` | 4 | Operational `k` fixed for phenotypic interpretability; set to `null` to use the silhouette-optimal `k` |
